...
# 手機殼原始廠牌欄中夾帶的產品線名稱。產品線須保留在 Product 名稱與分組鍵，
# 不轉成規格或特性詞條。
CASE_PRODUCT_LINES = {
    "DAPAD四角": "四角殼",
    "XMART皮套": "皮套",
...
def normalize_brand(brand_str, category):
    ...
    if not s:
        return None
    if category == CASE_CATEGORY and s in UNBRANDED_PRODUCT_LINES:
        return None
    if s in NON_BRANDS:
        return s
...
def is_resolvable_brand(brand_str):
    ...
    if brand_str is None:
        return True
    s = str(brand_str).strip()
    return s in UNBRANDED_PRODUCT_LINES or s not in NON_BRANDS
...
def parse_row(raw):
    """{Excel欄名: 原值} → 結構化 record(不碰 DB);空白列回 None。

    商品編碼可留空(尚未取得廠商條碼),此時 barcode 為 None,
    由 run_import 發自取碼;種類與廠牌都空白才視為無效列。

    record 欄位:
      barcode, category, brand_raw, brand, product_line, brand_resolvable,
    ...
    if barcode is None and category is None and brand_raw is None:
        return None
    ...
    return {
        ...
        "brand": brand,
        "product_line": (
            CASE_PRODUCT_LINES.get(brand_raw)
            if category == CASE_CATEGORY
            else GLASS_PRODUCT_LINES.get(brand_raw)
...
def product_key(rec):
    """款分組鍵:同 種類+廠牌+產品線 歸為一款。"""
    return (rec["category"], rec["brand"], rec.get("product_line"))


def product_name(rec):
    """款名:產品線優先使用「廠牌 產品線」,否則沿用「廠牌 種類」。"""
    if rec.get("product_line"):
        if rec["brand"]:
            return f"{rec['brand']} {rec['product_line']}"
        return rec["product_line"]
    if rec["brand"]:
...
GLASS_FRAME_FIELD = "框色"         # select:邊框顏色(白/黑),與鏡頭貼共用同一欄
...
def glass_layout(cat1_value):
    """分類1 → (版型值, 框色 or None)。
    滿版/空白→滿版;滿版(白)→滿版+框色白色;滿版(黑)→滿版+框色黑色。
    白/黑指的是玻璃邊框顏色,與鏡頭貼共用「框色」欄,不再當特性詞條。"""
    ...
    if s == "滿版(黑)":
        return "滿版", "黑色"
    return s, None                      # 未預期值:原樣當版型


def glass_attrs(spec_value, cat1_value):
    """整列鋼化玻璃規格 → {材質:[基礎詞...], 特性詞條:[詞條...], 版型:值, 框色:值}。"""
    bases, tags = glass_spec(spec_value)
    layout, frame = glass_layout(cat1_value)
    return {GLASS_SPEC_FIELD: bases, GLASS_TAGS_FIELD: tags,
            GLASS_LAYOUT_FIELD: layout, GLASS_FRAME_FIELD: frame}
...
F_SPEC = "規格"; F_CONNECTOR = "接頭"; F_LENGTH = "長度"
F_TAGS = "特性詞條"; F_SIZE = "尺寸"
# 產品型號:藍芽耳機與行動電源共用的品名 text 欄(值幾乎一物一值,不做 select);
# 刻意不叫「型號」以免與手機型號(PhoneModel)混淆。
F_PRODUCT_MODEL = "產品型號"
F_WATT = "瓦數"; F_PORTS = "孔位"; F_CAPACITY = "容量"

# 手機殼/鏡頭貼:規格、分類1 僅換欄名的純 select。
...
CATEGORY_FIELDS = {
    ...
    SOCKET_CATEGORY:   [(F_WATT, "select"), (F_PORTS, "select"), (F_COLOR, "select")],
    EARPHONE_CATEGORY: [(F_PRODUCT_MODEL, "text"), (F_COLOR, "select")],
    POWERBANK_CATEGORY: [(F_CAPACITY, "select"), (F_PRODUCT_MODEL, "text"),
                         (F_TAGS, "tags"), (F_COLOR, "select")],
...
# 行動電源規格字串內的功能詞 → 特性詞條。長詞在前(半固態 先於 固態),
# 命中即自字串移除,殘留字串為產品型號(品名)。
POWERBANK_TAG_WORDS = ["半固態", "固態", "磁吸", "支架", "Qi2",
                       "5合1", "8合1", "3合1", "4合1", "6合1", "7合1"]


def parse_powerbank_spec(spec_value):
    """行動電源規格 → {容量, 產品型號, 特性詞條:[...]}。

    先切出容量(/\\d+mAh/),再抽功能詞為詞條,殘留字串當產品型號(品名);
    殘留為空(整串都是功能詞)則產品型號 None。
    """
    out = {F_CAPACITY: None, F_PRODUCT_MODEL: None, F_TAGS: []}
    if spec_value is None:
        return out
    s = str(spec_value).strip()
    if not s:
        return out
    m = re.search(r"(\d+)\s*mAh", s, re.I)
    if m:
        out[F_CAPACITY] = f"{int(m.group(1))}mAh"
        s = (s[:m.start()] + " " + s[m.end():])
    # 品名保留原字串(只去容量):詞條另外標記,不從品名挖字,避免「超薄磁吸支架」
    # 被抽成「超薄」而失去辨識度。
    name = " ".join(s.split()).strip(" -")
    out[F_PRODUCT_MODEL] = name or None
    for word in POWERBANK_TAG_WORDS:
        if word.lower() not in name.lower():
            continue
        # 長詞優先:已收「半固態」就不再收其子字串「固態」
        if any(word != t and word in t for t in out[F_TAGS]):
            continue
        out[F_TAGS].append(word)
    return out


# ---- 插座:瓦數 / 孔位 拆解 ----

def parse_socket_spec(spec_value):
    """插座規格 → (瓦數, 孔位, warnings)。

    「33W 1A1C」→(「33W」,「1A1C」);「USB-2A」→(None,「2A」)。
    孔位去 USB- 前綴與空白後須為 數字+孔型(A/C/L) 的重複,否則不入孔位並列警告。
    """
    if spec_value is None:
        return None, None, []
    s = str(spec_value).strip()
    ...
    if m:
        watt = f"{int(m.group(1))}W"
        s = s[:m.start()] + " " + s[m.end():]
    rest = re.sub(r"USB\s*-?\s*", "", s, flags=re.I)
    rest = "".join(rest.split()).upper()
    if not rest:
        return watt, None, []
    if re.fullmatch(r"(\d+[ACL])+", rest):
        return watt, rest, []
    return watt, None, [f"插座孔位無法辨識:「{spec_value}」"]
...
def _writes_socket(rec, spec, cat1):
    """插座:規格拆瓦數/孔位(各自獨立 select),分類1 為顏色。"""
    opts, warns = [], []
    watt, ports, socket_warns = parse_socket_spec(spec)
    warns.extend(socket_warns)
    if watt is not None:
        opts.append((F_WATT, watt))
    if ports is not None:
        opts.append((F_PORTS, ports))
    if cat1 is not None:
...
def _writes_earphone(rec, spec, cat1):
    ...
    if rec.get("earphone_model"):
        texts.append((F_PRODUCT_MODEL, rec["earphone_model"]))
    if cat1 is not None:
        opts.append((F_COLOR, cat1))
...
def _writes_powerbank(rec, spec, cat1):
    """行動電源:規格拆容量(select)、產品型號(text)與功能詞條;顏色沿用原拆法。"""
    opts, texts = [], []
    pspec, pcolor = split_powerbank_spec(spec)
    parts = parse_powerbank_spec(pspec)
    if parts[F_CAPACITY] is not None:
        opts.append((F_CAPACITY, parts[F_CAPACITY]))
    if parts[F_PRODUCT_MODEL] is not None:
        texts.append((F_PRODUCT_MODEL, parts[F_PRODUCT_MODEL]))
    for tag in parts[F_TAGS]:
        opts.append((F_TAGS, tag))
...
CATEGORY_WRITERS = {
    SOCKET_CATEGORY: _writes_socket,
...
def _get_or_create_field(conn, name, category_id, field_type="select"):
    # AttributeField 已全域化;種類專屬性以 CategoryField 綁定表達(最小適配)。
    if category_id is None:
        row = conn.execute(
            "SELECT f.field_id FROM AttributeField f "
            "WHERE f.name=? AND f.field_type=? "
            "AND NOT EXISTS(SELECT 1 FROM CategoryField cf WHERE cf.field_id=f.field_id) "
            "ORDER BY f.field_id LIMIT 1", (name, field_type)).fetchone()
        if row:
            return row["field_id"]
        return conn.execute("INSERT INTO AttributeField(name,field_type) VALUES(?,?)",
                            (name, field_type)).lastrowid
    # 同名同型別的欄跨種類共用一份(AttributeField 為全域主檔),
    # 種類關係只掛 CategoryField;查無才新建。優先取未綁種類的種子欄。
    # ⚠️ tags(特性詞條)例外:各種類的詞彙互不相干(玻璃的抗AR vs 行動電源的Qi2),
    #    共用會讓詞條池互相汙染,故只重用已綁本種類的欄。
    if field_type == "tags":
        row = conn.execute(
            "SELECT f.field_id FROM AttributeField f "
            "JOIN CategoryField cf ON cf.field_id=f.field_id "
            "WHERE f.name=? AND f.field_type=? AND cf.category_id=? LIMIT 1",
            (name, field_type, category_id)).fetchone()
        if row:
            return row["field_id"]
    else:
        row = conn.execute(
            "SELECT f.field_id FROM AttributeField f WHERE f.name=? AND f.field_type=? "
            "ORDER BY (SELECT COUNT(*) FROM CategoryField cf WHERE cf.field_id=f.field_id), "
            "f.field_id LIMIT 1", (name, field_type)).fetchone()
    if row:
        fid = row["field_id"]
        if conn.execute("SELECT 1 FROM CategoryField WHERE category_id=? AND field_id=?",
                        (category_id, fid)).fetchone():
            return fid
    else:
        fid = conn.execute("INSERT INTO AttributeField(name,field_type) VALUES(?,?)",
                           (name, field_type)).lastrowid
    sort = conn.execute("SELECT COALESCE(MAX(sort),0)+1 FROM CategoryField "
                        "WHERE category_id=?", (category_id,)).fetchone()[0]
    conn.execute("INSERT OR IGNORE INTO CategoryField(category_id,field_id,sort) "
                 "VALUES(?,?,?)", (category_id, fid, sort))
    return fid
...
def _shared_field_id(conn, name):
    # 共用欄=全域且未綁定任何種類的欄位(如種子的商品描述、顏色)
    row = conn.execute(
        "SELECT f.field_id FROM AttributeField f WHERE f.name=? "
        "AND NOT EXISTS(SELECT 1 FROM CategoryField cf WHERE cf.field_id=f.field_id) "
        "ORDER BY f.field_id LIMIT 1", (name,)).fetchone()
...
def _ensure_glass_fields(conn, cid):
    ...
    frame_fid = _get_or_create_field(conn, GLASS_FRAME_FIELD, cid, "select")
    for i, fid in enumerate((spec_fid, tags_fid, layout_fid, frame_fid)):
        conn.execute("UPDATE CategoryField SET sort=? WHERE category_id=? AND field_id=?",
                     (100 + i, cid, fid))
    ...
    conn.execute("UPDATE CategoryField SET default_option_id=? WHERE category_id=? AND field_id=?",
                 (default_oid, cid, layout_fid))
    return spec_fid, tags_fid, layout_fid, frame_fid
...
def _next_store_barcode(conn):
    return next_store_barcode(conn)
...
def run_import(conn, records):
    ...
    glass_fids = {}       # category_id → (材質,特性詞條,版型,框色) field_id
    ...
    for rec in records:
        ...
        if category == GLASS_CATEGORY:
            ...
            spec_fid, tags_fid, layout_fid, frame_fid = glass_fids[cid]
            ...
            if ga[GLASS_FRAME_FIELD]:
                va_options.append((frame_fid, _get_or_create_option(
                    conn, frame_fid, ga[GLASS_FRAME_FIELD])))
        ...
        # 條碼處理:空白=尚未取得廠商條碼,直接發 TL;同一次匯入中首次出現走原條碼;
        # 第二次(含)出現=檔內重覆,改發 TL。
        bc = rec["barcode"]
        if bc is None:
            # 尚無廠商條碼:發自取碼。重跑安全——同 Product + 完全相同屬性/型號
            # 組合已有變體則跳過,不會每次重跑都再發一個新號。
            ...
            issued_blank += 1
            warnings.append(
                f"無廠商條碼,發自取碼「{tl}」:{product_name(rec)}")
            continue
        ...
        if occ == 1:
            ...
            if conn.execute("SELECT 1 FROM Barcode WHERE barcode=?", (bc,)).fetchone():
                ...
                continue
            pid = _resolve_product(conn, products, rec, cid, bid)
            src = "store" if has_store_barcode_prefix(bc) else "factory"
    ...
    serials = [parse_store_barcode(row["barcode"]) for row in
               conn.execute("SELECT barcode FROM Barcode WHERE barcode LIKE 'TL%'")]
    serials = [n for n in serials if n is not None]
    if serials:
        _bump_setting_min(conn, "next_store_barcode", max(serials) + 1)
...
def _print_report(stats, warnings):
    ...
    print(f"本次新增:變體 {stats['added_variants']}、條碼 {stats['added_barcodes']};"
          ...
          f"無廠商條碼發自取碼 {stats['issued_blank']}")
