...
**現況補記(2026-08-30)**：新增款式視窗已改為整頁垂直捲動,輸入區與表格
都不再內捲,本條的具體情境在該視窗不復存在;規則本身(同層選擇器優先權、
每層都要 `min-height: 0`)對其他子視窗仍然適用。

## VUE-19 會出現／消失的提示文字推動整排版面

**症狀**：新增款式的組合輸入區,規格欄選到第二個值(將產生 2 筆以上)的當下,
售價、原廠條碼、配自取碼那一排會跳動——條碼框上移、售價與勾選框下移,
原本對齊的兩個輸入框變成不齊。

**根因**：條碼欄下方的提示「請於預覽表逐筆掃描」是 `v-if` 隨筆數出現的,
它一出現就把該格撐高;而這排是 `align-items: end`(底部對齊),
一格變高就把同排其他格往下推。實測條碼框上移 7px、售價與勾選框下移 17px。

**解法**：讓會出現／消失的說明文字不佔版面——絕對定位在輸入框下方
(`position: absolute; top: 100%`),並在該排預留一行高度(`padding-bottom`)。
出現與否版面都不動。

**驗證方式**：量出現前後各元素的 `getBoundingClientRect().top`,
售價與條碼必須始終相同,該排到下一列的距離必須不變。

**相關**：同類型問題出現在任何「條件顯示的小字」旁邊有對齊需求的版面;
`align-items: end` 特別敏感。

**同源問題(候選選取器)**:`opt-picker` 的四塊(已選／候選／搜尋框／搜尋結果)
原本前後兩塊都是 `v-if` 有才顯示,選第一個值、開始打字都會把下面整排往下推;
被選走的候選 chip 又從候選區消失,行數再變一次。解法同樣是「不讓它影響版面」:
已選區 `min-height: 28px`／`max-height: 62px`,候選 chip 選走後留在原位只換樣式。
搜尋結果區保留一行高度、結果多就往下長(維護者裁示不要捲軸,寧可把下面的欄位往下推)。
⚠️ 這裡不要改用浮層(絕對定位下拉),維護者要的是版面完全不動、也不蓋住下方欄位。


## VUE-20 儲存格底色被兩套規則搶,修好一個問題卻換另一種顏色

**症狀**：新增款式的預覽表,把缺的型號補上之後,那一格的紅底退掉、
卻馬上變成藍底;使用者以為又出了別的問題。

**根因**：底色同時被兩件事使用——紅＝這格有錯,藍＝這欄各列的值不一樣(差異軸)。
兩者互相遮蔽,狀態一改就換色。另外整列紅底(dialog 公版 `.dialog-table tr.invalid`)
會把已經填好的欄位也染紅,看起來像每一格都有問題。

**解法**：一個視覺通道只表達一件事。底色只留給「這格要修」,差異高亮整套移除
(`cell-diff`／`diffFields`),整列紅底也不用;列層級的問題(與其他列重複)標在列上
(上下紅框線)。規則收斂成一句:有紅就是要修,沒紅就是可以建。

**相關**：同樣的取捨也適用於「用底色表示停用/唯讀」——本專案已否決過灰化呈現
(看起來像壞掉)。

## VUE-21 在 dialog 子視窗裡寫單一類別的樣式,被公版壓回去

**症狀**：預覽表的序號、操作、狀態三欄設了 `text-align: center` 卻沒有置中,
表頭欄名倒是置中了。

**根因**：`dialog-theme.css` 有一條 `.dialog-table th, .dialog-table td { text-align: left }`,
權重是兩個類別;自己寫的 `.batch-seq` 只有一個類別,權重不夠。表頭之所以生效,
是因為那條寫成 `.batch-table .dialog-table th`(權重剛好較高)。

**解法**：在子視窗裡覆寫公版時,選擇器要寫足層級——`.batch-table .dialog-table td.batch-seq`。
不要改用 `!important`(公版之後要改就查不出是誰壓的)。

**驗證方式**：改完在真實 pywebview 量 `getComputedStyle(td).textAlign`,
不要只看畫面猜。
